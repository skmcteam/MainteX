...
import { ScanLine, Keyboard, ArrowRight, Camera, CameraOff, Loader2 } from "lucide-react";
import { resolveAssetCode } from "@/app/(app)/assets/actions";
...
export function QRScanner() {
  ...
  const [status, setStatus] = useState<"idle" | "scanning" | "resolving" | "found" | "error">("idle");
  ...
  async function handleResult(text: string) {
    ...
    // QR codes generated by this app contain the full URL: /assets/<uuid>
    // Extract the UUID and navigate directly to the asset detail page.
    const urlMatch = trimmed.match(/\/assets\/([a-f0-9-]{36})/i);
    if (urlMatch) {
      router.push(`/assets/${urlMatch[1]}`);
      return;
    }

    // Treat the scanned text as an asset code and look it up in the DB.
    setStatus("resolving");
    const asset = await resolveAssetCode(trimmed);
    if (asset) {
      router.push(`/assets/${asset.id}`);
      return;
    }

    setErrorMsg(`ไม่พบรหัส "${trimmed}" ในระบบ`);
    setStatus("error");
  }
          ...
          {cameraError ? (
            ...
            <div className="flex flex-col items-center gap-3 p-4">
              <div className="flex items-center gap-2 text-xs" style={{ color: "var(--text-sub)" }}>
                {status === "resolving" ? (
                  <><Loader2 size={14} className="animate-spin" /><span>กำลังค้นหา...</span></>
                ) : (
                  <><ScanLine size={14} /><span>{status === "found" ? `พบ: ${lastScan}` : "กำลังสแกน..."}</span></>
                )}
              </div>
              {errorMsg && (
      ...
      {mode === "manual" && (
        <div className="panel-border rounded-2xl p-6">
          <form onSubmit={handleManualSubmit} className="flex flex-col gap-4">
            ...
            >
              {status === "resolving" ? <Loader2 size={16} className="animate-spin" /> : <ArrowRight size={16} />}
              {status === "resolving" ? "กำลังค้นหา..." : "เปิดอุปกรณ์"}
            </button>
          </form>
        </div>
      )}
